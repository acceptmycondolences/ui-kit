...
- Technology stack order follows `README.md`: Node.js, pnpm, Vite, React, TypeScript, ESLint, Prettier, Husky + lint-staged, Tailwind CSS, tw-animate-css, class-variance-authority, clsx, cmdk, input-otp, Radix UI, tailwind-merge, vaul, Storybook, Vitest + Playwright, vite-bundle-analyzer, @tailwindcss/vite, unplugin-dts.
- Use `pnpm` for package commands. The project requires Node.js `>=24.0.0` and `pnpm@11.3.0` according to `package.json`.
...
- The technology stack is intentionally not purely alphabetical. Keep this order:
  ...
  3. Vite plugin technologies from `vite.config.ts` plugin order, excluding `react()` because React is already part of the common baseline: vite-bundle-analyzer, @tailwindcss/vite, unplugin-dts.
...
- When changing packaging or tree-shaking behavior, inspect `package.json` exports, `vite.config.ts`, and runtime imports through `~/shared/config`, `~/shared/lib`, or `~/shared/ui`.
- For public component changes, update the colocated `*.stories.tsx` and `ui/README.md` when behavior or props change.
...
- Vite library entries map `src/shared/config/index.ts` to `dist/config.js`, `src/shared/lib/index.ts` to `dist/lib.js`, `src/shared/styles/index.ts` to `dist/styles/index.js`, `src/shared/ui/addons/index.ts` to `dist/ui/addons/index.js`, and `src/shared/ui/core/index.ts` to `dist/ui/core/index.js`.
...
- Keep `dist` as generated package output only; source changes belong under `src/shared`.
