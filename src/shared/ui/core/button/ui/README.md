...
При `disabled={true}` кнопка получает нативное состояние `disabled`: блокирует взаимодействие, исключается из последовательности фокуса и использует приглушенные цвета выбранного варианта.
...
При `isLoading={true}` содержимое кнопки заменяется индикатором загрузки. Кнопка получает нативное состояние `aria-busy="true"` и `disabled`, поэтому блокирует клики, клавиатурное взаимодействие и отправку формы, а также исключается из последовательности фокуса.

Loading-состояние сохраняет обычные цвета выбранного варианта. Приглушенные цвета `disabled`, включая `disabled:bg-constructive-disabled`, `disabled:text-constructive-foreground-disabled` и соответствующие классы остальных вариантов, не применяются. Это правило сохраняется и при одновременном использовании `disabled` и `isLoading`.
...
| :---------- | :-------------- | :----------: | :-------------------------------------------------------------------------------- |
| `isLoading` | `boolean`       |   `false`    | Показывает индикатор, нативно блокирует кнопку и сохраняет обычные цвета варианта |
...
Токены, используемые компонентом. Переопределяются на уровне темы через `:root` или `.dark`. Токены с суффиксом `-disabled` применяются только к обычному состоянию `disabled` и игнорируются при `isLoading={true}`.
